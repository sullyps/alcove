...
let util = {
  ...
  findDirSize: directory => {
    ...
    return Number(stdout.split(/\s+/)[0]);
  },
  ...
  countSubdirectories: directory => {
    fs.ensureDirSync(directory);
    return util.countSubdirectoriesExclude(directory, []);
  },

  /**
   * Utility method that returns the number of subdirectories in the
   * given directory (excluding directories containing an element
   * in the exclude array) as a number. (not recursive)
   * @param directory
   *   The target to inspect for subdirectories.
   * @param exclude
   *   An array of directory names to exclude.
   *   NOTE: A subdirectory will be excluded if an element of
   *   the exclude array is CONTAINED by the subdirectory.
   */
  countSubdirectoriesExclude: (directory, exclude) => {
    fs.ensureDirSync(directory);
    // TODO: Find platform independent alternatives
    let command = 'ls -l ' + directory + ' | grep "^d" | ';
    if (exclude.length !== 0)
    {
      command += 'grep -v ';
      for (let dir of exclude)
      {
        command += '-e ' + dir + ' ';
      }
      command += '| ';
    ...
    command += 'wc -l';
    let stdoutBuff = child_process.execSync(command);
    let stdout = stdoutBuff.toString();
