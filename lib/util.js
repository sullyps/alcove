...
let util = {
  ...
  /**
   * Returns human readable JSON information about a schedule given it's regex in the configuration
   * @param scheduleRegex The REGEX for the schedule specifed in the target machine config
   * @param gmtOffsetHours The GMT offest in hours to account for when providing the backup time
   * NOTE: This function is mainly for populating the frontend with data about a schedule
   * @see util.parseSchedule For other backend related functions
   */
  getInfoFromSchedule: (scheduleRegex, gmtOffsetHours) => {
    const weekdays = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    let backupEveryday = false;
    let backupEverydayExcept;

    let scheduledDays = scheduleRegex.split('|')[0].split(',')
    .map(dayIndex => {
      // For most days
      if (!isNaN(parseInt(dayIndex))) return weekdays[dayIndex]; 

      // For copy amount, formatted like '(<number>)' rather than '<number>'
      return null;
    })
    .filter(day => day); // Filters out null values

    const copyDayRegex = scheduleRegex.split('|')[1].split(';')[0]; // Ex. 1(4) means keep the last 4 copies from Monday
    const backupTimeUnparsed = scheduleRegex.split('[')[1].split(']')[0]; // Returns backup time in format nn:nn
    const copyDay = weekdays[copyDayRegex.split('(')[0]];
    const copyAmt = copyDayRegex.split('(')[1][0];

    let backupTime;

    // Account for GMT offset before providing backup time
    const hours = parseInt(backupTimeUnparsed.split(':')) + gmtOffsetHours; // parseInt will automatially remove leading zero
    const minutes = backupTimeUnparsed.split(':')[1]

    if (hours > 12) {
      const parsedHours = hours - 12;
      backupTime = parsedHours + ':' + minutes + 'PM';
    } else {
      backupTime = hours + ':' + minutes + 'AM';
    }

    if (!scheduledDays.includes(copyDay)) {
      const targetIndex = weekdays.findIndex(day => day === copyDay);

      // Ensure day is added to the correct chronological index
      // This effects how this is displayed on the frontend
      scheduledDays = [
        ...scheduledDays.slice(0, targetIndex),
        copyDay,
        ...scheduledDays.slice(targetIndex)
      ];
    }

    if (scheduledDays.length === 7) backupEveryday = true;

    if (scheduledDays.length === 6)
      backupEverydayExcept = weekdays.find(day => !scheduledDays.includes(day));
