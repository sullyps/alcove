...
//   example replacement JS function for findDirSize, but the performance is
...
let util = {
   ...
   * @return
   *   A Promise that will resolve with the size value as an integer (in 1K 
   *   blocks).
   */
  findDirSize: directory => {
    return new Promise((resolve, reject) => {
      child_process.exec('du -s ' + directory, (error, stdout, stderr) => {
  ...
  findFreeSpace: directory => {
    return new Promise((resolve, reject) => {
      child_process.exec('df -B1024 ' + directory, (error, stdout, stderr) => {
