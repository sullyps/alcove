 ...
 * TODO: Debug this function for when docker container creates key pairs.
 */
function getPublicSSHKeys(callback)
{
  var paths = [];

  // User may define a path to the identity file append
  ...
  // Need to search SSH protocols
  else
  {
    var types = ['id_dsa.pub', 'id_ecdsa.pub', 'id_ed25519.pub', 'id_rsa.pub'];
    types.forEach(function(type) {
      // Whereas in development, the Dockerfile should generate keys
      // and place them in a .ssh directory
      if (fs.existsSync('~/.ssh/' + type))
      {
        paths.push('~/.ssh/' + type);
      }
    });
  ...
  if (paths.length === 0)
  {
    ...
    return callback(new Error('No public key could be found'));
  }

  // Iterate over the paths to try to access public keys
  paths.forEach(function(path) {
    logger.debug('Attempting to get key from: ' + path);
    fs.readFile(path, function(err, key) {
