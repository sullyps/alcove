 ...
 * @param Date obj relativeDate
 *   The date from which the next scheduled time is relative to.
 */
function getNextScheduledTime(scheduleStr, relativeDate)
{
  var now = relativeDate;
  var schedule;
  ...
  schedule.daysSets.forEach(function(daySet) {
    daySet.days.forEach(function(dayOfWeek) {
      var date = new Date(relativeDate.getTime());
...
function generateBackupSummaryEmail() {
  ...
  message.shortMessage = 'Weekly backup Summary: Backup Size: ' + dataDirSize + '\n' +
  ...
  return db.ProcessEvent.findAll({
    ...
    header = 
      ...
      (recentStartTime.getHours() <= 12 ?
        recentStartTime.getHours() : recentStartTime.getHours()-12)  +
      ':' + recentStartTime.getMinutes() +
      (recentStartTime.getHours() < 12 ? 'am' : 'pm') + '\r\n' +
...
function sendSummaryEmail()
{
  ...
  .finally(function() {
    var nextScheduledTime = getNextScheduledTime(config.notifications.summary_schedule, new Date());
 ...
 *
 * @param callback
 *    Function that does something with the keys returned
 */
function getPublicSSHKeys(callback)
{
  var paths = [];

  // User may define a path to the identity file
  if (config.rsync.identity && fs.existsSync('/etc/backup/' + config.rsync.identity + '.pub'))
  {
    paths.push('/etc/backup/' + config.rsync.identity + '.pub');
  }
  // If no path is specified or incorrect path, need to search SSH protocols
  else
  {
    var types = ['id_dsa.pub', 'id_ecdsa.pub', 'id_ed25519.pub', 'id_rsa.pub'];
    types.forEach(function(type) {
      // In development, the Dockerfile should generate keys
      // and place them in a /home/node/.ssh directory
      logger.debug('Checking if /home/node/.ssh/' + type + ' exists');
      if (fs.existsSync('/home/node/.ssh/' + type))
      {
        paths.push('/home/node/.ssh/' + type);
        logger.debug('/home/node/.ssh/' + type + ' exists');
      }
    });
  }
  ...
  if (paths.length === 0)
  {
    logger.debug('Could not find a path containing public key!');
    return callback('No public key could be found');
  }
  else if (paths.length === 1)
  {
    logger.debug('Attempting to get key from: ' + paths[0]);
    fs.readFile(paths[0], function(err, key) {
  ...
  {
    return callback('More than one path to a public SSH key was found');
  }
}
  ...
  init: function(cfg, dbh) {
    ...
    .then(function(processEvent) {
      ...
      .then(function(processEvent) {
        ...
        // Print public ssh key so it can be used for authentication
        getPublicSSHKeys(function(err, keys) {
          if(err)
            logger.debug('Could not read SSH key: ' + err);
          else
            logger.info('Public SSH Key: ' + keys);
        });
      });
    })
    .catch(function (err) {
