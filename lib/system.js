...
function prepareMachines(config)
{
  ...
  for (var key in config.machines)
  {
    ...
    try
    {
      ...
      if (machine.extensionExclusion)
      {
        // Check file extensions to be ignored start with a period, prepend *
        machine.extensionExclusion.forEach(function(excludePattern, index) {
          if (excludePattern.match(/^\.([a-zA-Z\d]+)(?:$)/))
          {
            machine.extensionExclusion[index] = '*' + excludePattern;
          }
          else
          {
      ...
      if (machine.fileExclusion)
      {
        machine.fileExclusion.forEach(function(exclusionPattern, index) {
