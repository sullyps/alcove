...
function prepareMachines(config)
{
  ...
  for (let key in config.machines)
  {
    ...
    try
    {
      ...
      //
      // Ensure the identity specified (or defaulted to) has a proper pair, and
      // retain the location of the files in our config.
      // TODO
...
function generateBackupSummary() {
  ...
  let dataDirSize = util.getFormattedSize(util.findDirSize(config.data_dir));
  let freeSpace = util.getFormattedSize(util.findFreeSpace(config.data_dir));
  ...
  return db.ProcessEvent.findAll({
    ...
    header = 
      ...
      util.getFormattedDate(recentStartTime) + '\r\n' +
    ...
    for (let machineName in machines)
    {
      ...
      let numberBackups = util.countSubdirectories(path.join(config.data_dir, machineName));
      let diskSpace = util.getFormattedSize(util.findDirSize(path.join(config.data_dir, machineName)));
...
/**
 * Private helper function to find SSH keypair that will be used, and save
 * that pair in the config object for reference during the backup process.
...
function findIdentityKeyPair()
{
  ...
  paths.forEach(testPath => {
    ...
    {
      // TODO: test permissions on the private key too
      // Found one!
      logger.debug('Found a keypair: ', testPath);
      config.rsync.identity = testPath;
      try
      {
        key.file = testPath + '.pub';
        key.contents = fs.readFileSync(testPath + '.pub');
      }
      catch(err)
      {
  ...
  // TODO: We should bail on this error, but during the configuration validation (needs reorg)
  if (!key.file) key = { error: 'No public keys could be found' };
...
let system = {
  ...
  */
  init: (cfg, dbh) => {
    ...
    // Prepare the identity value, AND print the public key at INFO level so
    // that it can easily be copy-and-pasted into auth files.
    let key = findIdentityKeyPair();
    if (key.error)
    {
      logger.warn('Cannot identify any SSH key pairs... ' + 
        'It is likely that scheduled backups will fail with permission denied: ', key.error);
    }
    else
    {
      logger.info('The following public SSH key will be used for connections:');
      logger.info(key.file + '\n' + key.contents);
      logger.info('Remember to authorize this key on the target machine to enable backups');
    }
