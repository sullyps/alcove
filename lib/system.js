...
function findDirSize(directory)
{
  ...
  let stdoutBuff = child_process.execSync('du -hs ' + directory);
...
{
  // TODO: Find platform independent alternatives
  let stdoutBuff = child_process.execSync('df -h ' + config.data_dir);
...
function generateBackupSummaryEmail() {
  ...
  let dataDirSize = findDirSize(config.data_dir);
  let freeSpace = findFreeSpace();
      ...
      for (let j=0; j<backupEvents.length; j++) {
        ...
        if (data.machine === machine.name) 
        {
          message.longMessage += '  ' + 
            ...
            (data.transferSize/(1024*1024)).toFixed(2) + 'GB ' +
          ...
          let event = {
            ...
            size: (data.transferSize/(1024*1024*1024)).toFixed(2),
