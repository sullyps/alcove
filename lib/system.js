...
/**
 * Method that takes info from a backup try and inserts it into the db
 *     to create a history of attempts to reference later in things like a summary email.
 * @param machine
...
function insertBackupEvent (machine, backupInfo, rsyncExitCode) {
  ...
  db.BackupEvent.create({
    ...
    //machineLogger.debug('DB BackupEvent added (machine, rsyncExitCode, rsyncExitReason, transferSize, transferTimeSec)' +
    //'VALUES (' + machine.name + ',', rsyncExitCode + ',', exitReason + ',', backupInfo.totalTransferredFileSize + ',',
     //backupInfo.totalTransferTime + ')');
  ...
  */
  ...
  // TODO: Check to see that the last event was a kill event, if not notify via email.
    ...
    for (var i=0; i<machines.length; i++) {
      ...
      var timeoutTime = (env === 'development') ? 10 : nextBackup.getTime() - currentTime.getTime();
      machineLogger.info('Setting timeout for ' + timeoutTime + ' for backing up ' + machine.name);
    ...
    // Set timeout for weekly summary
    var summarySched = config.notifications.summary_email_sched;
    var currTime = new Date(Date.now());
    var nextScheduledTime = emailDispatcher.getNextSummaryEmailTime (summarySched, currTime);
    setTimeout(function() {emailDispatcher.backupSummaryEmail(machines)}, nextScheduledTime.getTime() - currTime.getTime());


    // TODO: Don't add this until making sure the config parameters are good.
    //     :    Most likely validate the parameters in the config file.
    db.ProcessEvent.create({
      'event' : 'start'
    }).then(function(processEvent) {
  ...
  /**
   * Getter method for the app variable.
   */
  getApp: function() {
    return app;
  },
  ...
  timeoutToNextBackup: function(machine, rsyncExitCode) {
    var attemptMultiplier = config.rsync.retry.time_multiplier;
    var maxAttempts = config.rsync.retry.max_attempts;
    var timeoutTime = 0;

    // TODO: Add other rsync exit codes to this first if block for no more attempts such as partial backup?
  ...
  fillBuckets: function (buckets, dir, machine, callback) {
    ...
    if (!backupDirStats.isDirectory()) {
      ...
      emailDispatcher.systemEmail('Missing Backup Directory Error (Do not reply)', errorMsg);
      logger.error(errorMsg);

      // TODO: Throw error here or let sys admin take care of it with the system still running?
      throw 'DirectoryDoesNotExistException'
    }
    fs.readdir(dir, function(err, backupsData) {
      if (err != null) {
        emailDispatcher.systemEmail('Backup System Error (Do not reply)', err);
        logger.error(err);
        throw err;
      }
  ...
  backupProcess: function(machine) {
    ...
    if (backupQueues.inProgress.length < config.rsync.max_backups) {
      ...
      machineLogger.debug('Finding the backup schedule.');
      ...
      system.fillBuckets(buckets, backupDirectory, machine, function() {
        if (buckets[buckets.length-1].backup == null) {
          ...
          rsync.runRsync(machine, function(error, code, rsyncInfo) {
            ...
            if (code === 0) {
              ...
              // If email notifications is set up send the email that there was an error.
            ...
            machine.backupTimeoutId = setTimeout(function() { system.backupProcess(machine) }, nextBackup);
        ...
        // TODO: I DO  NOT LIKE  THIS ELSE STATEMENT CALLING THE SAME THING AS ABOVE.
          ...
          // TODO: on second thought this gets called when there is a partial backup because of an error as well.
