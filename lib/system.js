...
function prepareMachines(config)
{
  ...
  for (var key in config.machines)
  {
    ...
    var machineLogger = logging.getLogger(machineConfig.name);
    ...
    try
    {
      ...
      machineConfig.ignore_extensions.forEach(function(setting) {
      ...
      machineConfig.backup_directories.forEach(function(setting) {
        ...
        fullpath.split(path.sep).forEach(function(pathComponent, idx, splits) {
...
function generateBackupSummaryEmail() {
  ...
  return db.ProcessEvent.findAll({
    ...
    message.htmlMessage = nj.configure('./lib/templates').render('email.njk', emailData);
