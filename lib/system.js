  ...
  */
  init: function(cfg, dbh) {
    ...
    Object.keys(machines).forEach(function(name) {
      ...
      {
        logger.error('Could not schedule backup for "' + name + '": ' + error.message);
        logger.debug(error.stack);
      }
    });
  ...
  fillBuckets: function (buckets, dir, machine, callback) {
    ...
    fs.readdir(dir, function(err, backupsData) {
      ...
      // TODO: what about inprogress? what about incorrectly named directories?
  ...
  backupProcess: function(machine) {
    var machineLogger = logging.getLogger(machine.name);
    // TODO: Do this check within the rsync process after filled buckets
        ...
        backupQueues.inProgress.length < config.rsync.max_simultaneous) 
    {
      ...
      // TODO: Don't mark as in progress unless there is a scheduled backup
      // TODO: Reconfigure async try catch process (Use promises?)
      // TODO: Create internal function for completion of backup (start next in queue)
      backupQueues.inProgress.push(machine);
      var backupDirectory = path.join(config.data_dir, machine.name);

      // Get a list of schedule "buckets"
      var buckets = system.getBuckets(machine.schedule, new Date());
      ...
      try
      {
        system.fillBuckets(buckets, backupDirectory, machine, function() {
          if (buckets[buckets.length - 1].backup == null)
          {
            ...
            try
            {
              rsync.runRsync(config, machine, function(error, code, rsyncInfo) {
