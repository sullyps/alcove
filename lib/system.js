...
function prepareMachines(config)
{
  ...
  for (var key in config.machines)
  {
    ...
    try
    {
      ...
      topLevelPatterns.forEach(function(inclusionPattern, index) {
        if (path.isAbsolute(inclusionPattern))
        {
          topLevelPatterns[index] = path.join(inclusionPattern, '**');
        }
        else
        {
      ...
      extensionExclusion.forEach(function(excludePattern, index) {
        if (excludePattern.match(/^\.([a-zA-Z\d]+)(?:$)/))
        {
          extensionExclusion[index] = '*' + excludePattern;
        }
        else
        {
      ...
      fileExclusion.forEach(function(exclusionPattern, index) {
