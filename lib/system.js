    ...
    try
    {
      ...
      if (machineConfig.extensionExclusion)
      {
        ...
        machineConfig.extensionExclusion.forEach(function(excludePattern, index) {
          ...
          {
            machineConfig.extensionExclusion[index] = '*' + excludePattern;
          }
          else
          {
            throw new Error('Error in: ' + excludePattern +
              '\nIs not a proper extension in config for: ' + machine.name);
          }
        });
      }
      else
      ...
      machine.topLevelInclusion.forEach(function(inclusionPattern, index) {
        ...
        {
          machine.topLevelInclusion[index] = path.join(inclusionPattern, '***');
        }
        else
        {
