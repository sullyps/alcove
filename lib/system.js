 ...
 * @param backupRequested
 *   Whether or not the event that's to be inserted originates from a user request
 *   rather than a schedule.
 */
function insertBackupEvent(machine, bucket, rsyncStats, backupRequested = false) {
 ...
 * @param requested
 *   Whether or not the backup that is to be completed originates from a user request or from a schedule
 *   (Defaults to false)
 */
function completeBackup(rsyncErr, machine, newBuckets, rsyncStats, toDelete, backupRequested = false)
{
  let slack;
  ...
  insertBackupEvent(machine, newBuckets[newBuckets.length - 1].date.toString(), rsyncStats, backupRequested);
  ...
  else
  {
    ...
    toDelete.forEach( dir => {
      ...
      try
      {
        //fs.removeSync(dir); // TODO: Tweak this to not delete folders from requested copies
      }
      catch (rmError)
      {
...
let system = {
  ...
  */
   ...
   *  @params
   *    machine-machine that is scheduled to backup (usually needs machine object, only needs name for manual request).
   *    backupRequested-whether or not this backup was manually requested. (defaults to false)
   */
  backupProcess: (machine, backupRequested = false) => {
    let slack;
    if (config.notifications.slack.webhook && config.notifications.slack.level === 'all')
    {
      slack = require('./notifications/slack')();
    ...
    system.fillBuckets(newBuckets, machineDir, machine, (bucketErr, backupNeeded, toDelete) => {
      ...
      // backupNeeded should always be set to true if this is a requested backup.
      if (backupRequested)
        backupNeeded = true;
      ...
      if (backupNeeded)
      {
            ...
            backupQueues.inProgress.length < config.rsync.max_simultaneous) 
        {
          ...
          rsync.runRsync(config, machine, backupRequested, (rsyncErr, rsyncStats) => {
            if (rsyncErr)
            {
            ...
            return completeBackup(rsyncErr, machine, newBuckets, rsyncStats, toDelete, backupRequested);
          });
        }
        else
        {
...
exports.completeBackup = completeBackup;
