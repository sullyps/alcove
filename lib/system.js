...
function completeBackup(rsyncErr, machine, rsyncStats, toDelete)
{
  ...
  if (rsyncErr && machine.failures.length == maxAttempts)
  {
    ...
    // TODO Reword this
    notifications.dispatch('Failed backup for "' + machine.name + '"', 
        'The backup for "' + machine.name + '" could not be completed: ' + rsync.error + '\n' +
   ...
   * @param directory to find size of
   */
  ...
  // TODO: Find platform independent alternatives
  // NOTE: Will this command execute as expected?
   ...
   * @param schedule
   *   the string schedule, from config.notifications.summary_sched
   * @param date
  ...
  function getLastSummaryEmailTime(schedule, date){
    ...
    if (!found) {
      numberOfDaysAgo = day + 7 - scheduledDays[scheduleDays.length-1];
    }
  ...
  /**
   * Private helper method to create weekly summary email. 
   ...
   */
  function generateBackupSummaryEmail(db, machines, config){
    ...
    db.ProcessEvent.findOne({
      ...
      emailSummary +=
         ...
         // TODO: add date the email is sent out
         '-----\n' +
         'Total size of all backups: ' + backupsDirSize + '\n' +
         'Disk space remaining: ' + freeSpace + '\n' +
         "Last 'start' ProcessEvent: " + processEvent[0].eventTime +
         '-----\n' +
         // TODO: read through documentation to find how to report
         'Failed machines: \n' +
         '-----\n\n';
    });

    var summarySchedule = config.notifications.summary_email_sched;
    var lastSummaryDate = getLastSummaryEmailTime(summarySchedule, new Date(Date.now()));
    ...
    db.BackupEvent.findAll({
      where: {
        ...
        for (var i=0; i<machines.length; i++) {
          ...
          emailSummary +='Summary for ' +  machine.name + ':\n';
  ...
  /**
   * Private helper method to create the subject and short summary
   * of the weekly backups that have occurred
   */
  // TODO: Improve this function name
  function generateBackupSummaryShortSub(config){
    var subject = 'Weekly backup summary';
    var backupsDirSize = findFileSize(config.rsync.destination_dir);
    var freeSpace = findFreeSpace();

    var shortSummary = '';
    // TODO: Add date to short summary
    shortSummary += 'Ttl Backup Sz: ' + backupDirSize + '\n' +
                    'Ttl Free Space: ' + freeSpace + '\n';

    return [subject,shortSummary];
  }
  
  /** 
   * Private helper method to create object with attributes for different
   * types of messages
   * Attributes: subject - "shortest text" 
   *             shortMessage - "140 char or less"
   *             longMessage - "text format detailed message"
   *             htmlMessage - "html styled long message"
   * TODO: Finish parameter documentation
  // TODO: Create functions for the subject heading (highlight of backups)
  // TODO: Create function to generate < 140 char message about backups
  */
  function generateMessage(db, machines,config){
    return {
      subject : generateBackupSummaryShortSub(config)[0],
      shortMessage : generateBackupSummaryShortSub(config)[1],
      longMessage : generateBackupSummaryEmail(db,machines,config),
      // TODO: complete function to generate html message
      html : '',
    };
...
var system = {
  ...
  */
  init: function(cfg, dbh) {
    ...
    if (config.summary)
    {
      var nextScheduledTime = getNextScheduledTime(config.summary);
    ...
    .then(function(processEvent) {
      ...
      if (processEvent && processEvent.event !== 'exit')
      {
        ...
        notifications.dispatch(subject, text);
      }
