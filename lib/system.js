...
function generateBackupSummaryEmail() {
  ...
  .then(function() {
    ...
    return db.BackupEvent.findAll({
      where: {
        backupTime: {
          // FIXME: the gt operator returns the query = instead of >
  ...
  init: function(cfg, dbh) {
    ...
    Object.keys(machines).forEach(function(name) {
      ...
      try
      {
        ...
        {
          logger.info('A backup event will occur in 3 minutes. Please' +
          ...
          nextBackup = new Date(currentTime.getTime() + 3 * 60000);
      ...
      catch (error)
      {
        notifications.dispatch({
          ...
          longMessage : '*** CRITICAL machine: "' + name + '" will not be' +
