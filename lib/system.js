...
let system = {
  ...
  */
   ...
   *   an improper completion.
   */
  insertRequestedBackupEvent: (machine, rsyncStats) => {
  ...
  fillBuckets: (buckets, backupDir, machine, callback) => {
    ...
    fs.readdir(backupDir, (err, dirListing) => {
      ...
      dirListing.forEach( dirname => {
        ...
        {
          if (dirname === 'requested') return; // This is an intended directory, no need to log this warning
  ...
  /**
   * Gets all RequestedBackupEvents from the database for the specified machine
   * @param machineName The name of the machine that will be searched for RequestedBackupEvents
   */
  getRequestedBackupEvents: (machineName) => {
    return db.RequestedBackupEvent.findAll({
      where: {
        machine: machineName
      }
    });
