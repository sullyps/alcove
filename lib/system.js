...
module.exports = system = {
  ...
  // TODO: REMOVE backups from this method signature and just go through the directory in the backupsPath to find the list of backups to remove.
  removeDirectories: function (backups, buckets, backupsPath) {
    logger.info('removing directories not contained in any buckets.');
    system.getRemovedDirectoriesList(backups, buckets, function(removedDirectories) {
      removedDirectories.forEach(function(unusedBackupDir) {
  ...
  backupProcess: function(machine) {
    ...
    system.fillBuckets(buckets, backupDirectory, machine, function() {
      ...
      runRsync(machine, function() {
        logger.info('removing directories');
        system.removeDirectories(backups, buckets, backupDirectory);
      });
    });
    var currentTime = new Date(Date.now());
    var nextBackup = system.findNextScheduledTime(machine.schedule, currentTime);
