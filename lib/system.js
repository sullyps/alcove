  ...
  for (var key in config.machines)
  {
    ...
    try
    {
      ...
      var machine = {
        ...
        ip: machineConfig.ip,
        // if not specified, use /home and /etc as top level inclusion patterns
        topLevelInclusion: (machineConfig.topLevelInclusion ? machineConfig.topLevelInclusion : ['/home','/etc']),
      ...
      machine.topLevelInclusion.forEach(function(inclusionPattern, index) {
        if (path.isAbsolute(inclusionPattern))
        {
          machine.topLevelInclusion[index] = "'" +
