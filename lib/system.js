  ...
  */
  init: function(application) {
    ...
    for (var i=0; i<machines.length; i++) {
      ...
      machine.backupTimeoutObject = setTimeout(function() {system.backupProcess(this);}.bind(machine), timeoutTime);
//       Start the systems process by setting the timeout until the machines scheduled backup time.
//      (function(machine) {
//        var timeout = setTimeout(function() {system.backupProcess(machine);}, timeoutTime);
//        machine.backupTimeoutObject = timeout;
//      })(machine);
    }

    // Set the timeout for the backup summary emails.
    ...
    db.ProcessEvent.findAll({
        ...
        order: [
          ...
          {
            var processEvent = processEvents[0];
            if (processEvent.event !== 'exit')
            {
              if (config.notifications.receive_email_notifications)
              {
                var subject = 'Backup System Unexpected Shutdown Occurred';
                var text = 'The backup system restarted after an unexpected shutdown.  Some backups may be missing';
                emailDispatcher.systemEmail(subject, text);
              }
            }
          }
        }).catch(function (err) {
          logger.error('Error occurred reading from ProcessEvents table in the database: ' + err);
        });
