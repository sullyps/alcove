...
function runRsync(machine) {
  if (backupInfo.backupCount < backupConfig.maxSimultaneousBackups) {
    backupInfo.backupCount++;
    rsync(machine, backupInfo);
...
module.exports = system = {
  ...
  // TODO: add rsync back in as a call back so that for testing purposes we don't need to call it.
  fillBuckets: function (buckets, dir, machine, removeDirs) {
    var dateArr = [];
    var backups = [];
    var date = new Date();
  ...
  backupProcess: function(machine) {
    ...
    system.fillBuckets(buckets, backupDirectory, machine, system.removeDirectories);
    var currentTime = new Date(Date.now());
    var nextBackup = system.findNextScheduledTime(machine.schedule, currentTime);
