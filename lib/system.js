...
function getPublicSSHKeys(callback)
{
  ...
  else
  {
    ...
    TYPES.forEach(function(type) {
      ...
      // and place them in a HOME/.ssh directory
      var sshDir = path.join(homeDir, '.ssh', type);
      logger.debug('Checking if' + sshDir + ' exists');
      if (fs.existsSync(sshDir))
      {
        paths.push(sshDir);
        logger.debug(sshDir + ' exists');
      }
    });
  }
    ...
    .then(function(processEvent) {
      ...
      .then(function(processEvent) {
        ...
        getPublicSSHKeys(function(err, keys) {
          if(err)
            logger.error('Could not read SSH key: ' + err);
          else
