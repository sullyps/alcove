...
function prepareMachines(config)
{
  ...
  for (let key in config.machines)
  {
    ...
    try
    {
      ...
      // TODO: only record backup dir sizes? check with startup time? etc? Skip this during startup? Mark as approx?
  ...
  */
  ...
  backupProcess: machine => {
    ...
    try
    {
      ...
      buckets = getBuckets(machine.schedule, new Date());
    }
    catch (bucketError)
    {
