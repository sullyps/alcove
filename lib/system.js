...
function insertBackupEvent (machine, rsyncStats) {
  ...
  db.BackupEvent.create({
    ...
    'rsyncExitReason' : (rsyncStats.error ? rsyncStats.error.Error : ''),
...
function prepareMachines(config)
{
  ...
  for (let key in config.machines)
  {
    ...
    try
    {
      ...
      machineConfig.backup_directories.forEach( setting => {
        ...
        fullpath.split(path.sep).forEach((pathComponent, idx, splits) => {
...
function findFreeSpace()
{
  ...
  // TODO: Find platform independent alternatives
  let stdoutBuff = child_process.execSync('df -B1 ' + config.data_dir);
...
/**
 * Private helper method that returns the number of subdirectories in the
 * given directory as a number. (not recursive)
 * @param directory
 *   The target to inspect for subdirectories.
 */
function findSubdirectories(directory) {
...
/**
 * Private helper function that takes an integer number of bytes and
...
function generateBackupSummaryEmail() {
  ...
  let dataDirSize = getFormattedSize(findDirSize(config.data_dir));
  let freeSpace = getFormattedSize(findFreeSpace());
  let summarySchedule = config.notifications.summary_schedule;
  let lastSummaryDate = getLastSummaryEmailTime(summarySchedule, new Date());
  let header = '';
  let details = '';
  let emailData;
  ...
  return db.ProcessEvent.findAll({
    ...
    let recentStartTime = processEvents[0].eventTime;
    header = 
      'Weekly Backup Summary\r\n' +
      'Backups from: ' + getFormattedDate(lastSummaryDate).substring(0, 10) + ' to ' +
      getFormattedDate(new Date()).substring(0, 10) + '\r\n' +
      '=====\r\n' +
      'Total Disk Space Used: ' + dataDirSize + '\r\n' +
      'Disk Space Remaining: ' + freeSpace + '\r\n' +
      'Last Backup System Startup: ' +
      getFormattedDate(recentStartTime) + '\r\n' +
      'Backup System Startups: ' + processEvents.length + '\r\n';
    details =
      'Weekly Backup Summary: Backup Size: ' + dataDirSize + '\n' +
      'Free Space: ' + freeSpace + '\n' +
      'Startups: ' + processEvents.length + '\n';
    emailData = {
      fromDate: getFormattedDate(lastSummaryDate).substring(0, 10),
      toDate: getFormattedDate(new Date()).substring(0, 10),
      totalSize: dataDirSize,
      freeSpace: freeSpace,
      lastStartDate: getFormattedDate(recentStartTime).substring(0, 10),
      lastStartTime: getFormattedDate(recentStartTime).substring(11),
      startEvents: processEvents.length
    };
  }).then( () => {
    return db.ProcessEvent.findAll({
      where: {
  ...
  }).then( stopProcessEvents => {
    header +=
      'Backup System Shutdowns: ' + stopProcessEvents.length + '\r\n' +
       '=====\r\n\r\n';
    details += 'Shutdowns: ' + stopProcessEvents.length;
    emailData.stopEvents = stopProcessEvents.length;
  }).then( () => {
    ...
    for (let machineName in machines)
    {
      ...
      let numberBackups = findSubdirectories(path.join(config.data_dir, machineName));
      let diskSpace = getFormattedSize(findDirSize(path.join(config.data_dir, machineName)));
      ...
      for (let j=0; j<backupEvents.length; j++) {
        ...
        if (data.machine === machine.name) 
        {
          message.longMessage += '  ' + 
            ...
            getFormattedSize(data.transferSize) +
            ' were backed up in ' + (data.transferTimeSec/60).toFixed(2) +
            ' min';
          let event = {
            date: getFormattedDate(data.backupTime).substring(0, 10),
            time: getFormattedDate(data.backupTime).substring(11),
            size: getFormattedSize(data.transferSize),
            minutes: (data.transferTimeSec/60).toFixed(2),
    ...
    logger.debug('Number of machines in Summary Notification: ' +
...
{
  generateBackupSummaryEmail().then( message => {
 ...
 * Checks if user has provided a path to the keys in the config file, then
 * searches through default locations to identify existing keys that SSH will
 * automatically use if present.
 */
function getPublicSSHKeys()
{
  ...
  {
    // These defaults are defined within the man page of SSH
    ...
    TYPES.forEach( type => {
      let sshPublicKey = path.join(homeDir, '.ssh', type);
      logger.trace('Checking if "' + sshPublicKey + '" exists');
  ...
  // Construct our return response
  if (paths.length === 0)
  {
    keys.error = 'No public keys could be found\n';
    return keys;
  }
  paths.forEach( path => {
    logger.debug('Attempting to read public key from: ' + path);
    let fileContents;
    try
    {
...
let system = {
  ...
  */
  init: (cfg, dbh) => {
    ...
    let keys = getPublicSSHKeys();
    if (keys.error)
    {
      logger.warn('Cannot access any public SSH keys for this user. ' + 
        'It is likely that scheduled backups will fail with permission denied: ' + keys.error);
    }
    else
    {
      logger.info('The following SSH keys will be used for connections:');
      for (let file in keys)
      {
        logger.info(file + '\n' + keys[file]);
      }
      logger.debug('Remember to authorize at least one of the above keys on the target machine to enable backups');
    }
   ...
   */
  addBucketToList: (bucket, bucketList) => {
    let added = false;
   ...
   *     current date.
   */
  getBuckets: (sched, date) => {
    let schedObj = system.parseSchedule(sched);
  ...
  backupProcess: machine => {
    ...
    system.fillBuckets(buckets, machineDir, machine, (bucketErr, backupNeeded, toDelete) => {
      if (bucketErr)
      {
