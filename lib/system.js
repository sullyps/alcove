...
function generateBackupSummaryEmail() {
  ...
  return db.ProcessEvent.findAll({
    ...
    header = 
      ...
      "Backups from: " + lastSummaryDate.getYear() + '-' + (lastSummaryDate.getMonth() + 1) +
        '-' + lastSummaryDate.getDay() + ' to ' + getNextScheduledTime(summarySchedule) + '\r\n' +
      '=====\r\n' +
      'Total size of all backups: ' + dataDirSize + '\r\n' +
      'Disk space remaining: ' + freeSpace + '\r\n' +
      // TODO: Format this date
      'Running since: ' + processEvents[0].eventTime + '\r\n' +
    ...
    Object.keys(machines).forEach(function(name) {
      ...
      try
      {
        ...
        if(timeoutTime > 4 * (60) * (60000))
        {
          ...
          nextBackup = new Date(currentTime.getTime() + 60000);
