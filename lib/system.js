...
let sizingQueues = { remaining: [], inProgress: [] };
let machines = {};
...
function insertBackupEvent(machine, bucket, rsyncStats) {
  ...
  db.BackupEvent.create({
    ...
    'size': null  // TODO: add this to the sizingQueue
  })
  .then( backupInfo => {
...
function prepareMachines(config)
{
  ...
  for (let key in config.machines)
  {
    ...
    try
    {
      ...
      // Read list of relevant previous events from DB
      // TODO
 ...
 * @param bucket
 *   The string representation of the bucket this backup attempt was targeting.
 * @param rsyncStats
 ...
 */
function completeBackup(rsyncErr, machine, bucket, rsyncStats, toDelete)
{
  // Insert backup event
  insertBackupEvent(machine, bucket, rsyncStats);
...
function generateBackupSummary() {
  ...
  // TODO: Use the memory models to get these values
  // Old calls:
  //util.getFormattedSize(util.findDirSize(config.data_dir));
  //util.getFormattedSize(util.findFreeSpace(config.data_dir));
  let dataDirSize = 'TODO';
  let freeSpace = 'TODO';
  ...
  return db.ProcessEvent.findAll({
    ...
    // TODO: handle an empty array for processEvents or switch to findOne()
    ...
    for (let machineName in machines)
    {
      ...
      let numberBackups = 'TODO';
      let diskSpace = 'TODO';
      // TODO: use the memory models for a recent value
      // Old calls were:
      //util.getFormattedSize(util.findDirSize(path.join(config.data_dir, machineName)));
      //util.countSubdirectories(path.join(config.data_dir, machineName));
...
let system = {
  ...
  */
      ...
      if (backupNeeded)
      {
            ...
            backupQueues.inProgress.length < config.rsync.max_simultaneous) 
        {
          ...
          rsync.runRsync(config, machine, (rsyncErr, rsyncStats) => {
            ...
            return completeBackup(rsyncErr, machine, buckets[buckets.length - 1].date.toString(), rsyncStats, toDelete);
          });
        }
        else
        {
  ...
  /**
   * Get the information about system disk usage.
   * @return
   ...
   *   + time : The timeistamp at which the last size measurement was completed.
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let machineName in machines)
      {
        ...
        else
        {
          ...
          time = Math.min(time, machines[machineName].totalSize.when);
