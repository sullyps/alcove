...
function getLastSummaryEmailTime(schedule, date){ 
  ...
  // FIXME: Logic fails for schedule = '1,3;[8:00]' and date is Mon at 7:59:59
  // The last summary email time should be the previous Wednesday at 8:00
  // This logic returns previous Mon at 8:00 as last summary time
  while (i >= 0 && !found) {
  ...
  init: function(cfg, dbh) {
    ...
    Object.keys(machines).forEach(function(name) {
      ...
      try
      {
        ...
        // NOTE: arbitrary time of 4 hours chosen
        if(timeoutTime > 4 * (60) * (60000))
        {
          logger.info('A backup event will occur in 1 minutes. Please' +
            ' shut down or cancel program to stop backup from occurring');
          // setTimeout(function() {
          //   logger.info('2 minutes until backup is performed');
          // }, 60000);
          // setTimeout(function() {
          //   logger.info('1 minute until backup is performed');
          // }, 2 * 60000);
          nextBackup = new Date(currentTime.getTime() + 1 * 60000);
          machineLogger.info('Scheduling backup at ' + nextBackup);
          machine.timeoutId = setTimeout(function() {
            system.backupProcess(this);
          }.bind(machine), 3 * 60000);
        }
        else
        {
