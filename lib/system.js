 ...
 * NOTE: This is my attempt at finding the public ssh keys to print to the
 * console.
 */
function getPublicSSHKeys(callback)
{
  ...
  {
    var types[] = ['id_dsa.pubs', 'id_ecdsa.pub', 'id_ed25519.pub', 'id_rsa.pub'];
    types.forEach(function(type) {
      if (fs.existsSync('/etc/backup/.ssh/' + type))
      {
        paths.push('/etc/backup/.ssh/' + type);
      }
    });
  }
  ...
  init: function(cfg, dbh) {
    ...
    .then(function(processEvent) {
      ...
      .then(function(processEvent) {
        ...
        getPublicSSHKey(function(err, keys) {
