...
function prepareMachines(config)
{
  ...
  for (var key in config.machines)
  {
    ...
    try
    {
      ...
      machineConfig.extensionExclusion.forEach(function(setting) {
        machine.extensionExclusion.push('*' + setting);
      });
      ...
      machineConfig.topLevelInclusion.forEach(function(setting) {
        ...
        fullpath.split(path.sep).forEach(function(pathComponent, idx, splits) {
          ...
          if (partial === (path.sep + fullpath))
          {
            ...
            machine.topLevelInclusion.push(path.join(partial, "***"));
          }
          else
          {
            ...
            if (!machine.topLevelInclusion.includes(partial) && !machine.topLevelInclusion.includes(path.join(partial, "***"))) 
              machine.topLevelInclusion.push(partial);
          }
        });
      });
