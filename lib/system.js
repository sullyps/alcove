...
function completeBackup(rsyncErr, machine, rsyncStats, toDelete)
{
  ...
  else if (rsyncErr)
  {
    ...
    slack.send({
      longMessage: `*${new Date()}* Backup failed for machine "${machine.name}". ` +
          `Retry attempt scheduled for ${util.getFormattedTimespan(timeout)} from now.`
    });
  }
  else
  {
    slack.send({
      longMessage: `*${new Date()}* Backup succeeded for machine "${machine.name}".`
    });
...
let system = {
  ...
  */
  ...
  backupProcess: machine => {
    ...
    let machineLogger = logging.getLogger(machine.name);
    slack.send({
      longMessage: `*${new Date()}* Attempting to back up machine "${machine.name}"...`
    });
    ...
    catch (bucketError)
    {
      ...
      slack.send({
        longMessage: `*${new Date()}* Cannot get list of schedule backup times: ${bucketError.message}\n` +
            `*CRITICAL: this machine will not be schedule for additional backups until this problem is resolved*`
      });
      return;
    }
    ...
    system.fillBuckets(buckets, machineDir, machine, (bucketErr, backupNeeded, toDelete) => {
      if (bucketErr)
      {
        ...
        slack.send({
          longMessage: `*${new Date()}* Could not assign existing backup directories to previously scheduled ` +
              `backup times: ${bucketErr.message}\n` +
              `*CRITICAL: this machine will not be schedule for additional backups until this problem is resolved*`
        });
        return;
      }
