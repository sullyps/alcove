...
function prepareMachines(config)
{
  ...
  for (var key in config.machines)
  {
    ...
    try
    {
      ...
      // FIXME: This validation should be placed within the config
      // Check all top level directories are absolute paths, append /***
      // Triple asterisks will grab all subdirectories from the top level
      // directories.
      // TODO: The following error checking and transformation will likely be moved
      // into config parsing.
      if (machineConfig.extensionExclusion)
      {
        // Check file extensions to be ignored start with a period, prepend *
        machineConfig.extensionExclusion.forEach(function(excludePattern, index) {
          if (excludePattern.match(/^\.([a-zA-Z\d]+)(?:$)/))
          {
            machineConfig.extensionExclusion[index] ="'" + '*' +
              excludePattern + "'";
          }
          else
          {
            throw new Error('"' + excludePattern +
              '" is not a proper extension in config for: ' + machineConfig.name);
          }
        });
      }
      else
        machineConfig.extensionExclusion = '';

      // Check that file/directories to be ignored are absolute paths
      // not containing any wildcards
      // TODO: Check these do not contain any *
      if (machineConfig.fileExclusion)
      {
        machineConfig.fileExclusion.forEach(function(exclusionPattern, index) {
          if (!path.isAbsolute(exclusionPattern))
          {
            throw new Error('Error in: ' + exclusionPattern +
              '\nIs not an absolute path in config for: ' + machineConfig.name);
          }
        });
      }
      else
        machineConfig.fileExclusion = '';
      // TODO: Check if directory contains any *
      machineConfig.topLevelInclusion.forEach(function(inclusionPattern, index) {
        if (path.isAbsolute(inclusionPattern))
        {
          machineConfig.topLevelInclusion[index] = "'" +
            path.join(inclusionPattern, '***') + "'";
        }
        else
        {
          throw new Error('Error in: ' + inclusionPattern +
            '\nIs not an absolute path in config for: ' + machine.name);
        }
      });
      ...
      var machine = {
        ...
        ip: machineConfig.ip,
        topLevelInclusion: machineConfig.topLevelInclusion,
        extensionExclusion: machineConfig.extensionExclusion,
        fileExclusion: machineConfig.fileExclusion,
      ...
      try
      {
        ...
        // Successfully scheduled backup call, not retain this machine object in memory
    ...
    {
      logger.error('Error preparing "' + key + '" for backup: ' + error.message);
      logger.debug(error.stack);
