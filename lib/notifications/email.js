...
function init(tag, to, from, smtp)
{
  ...
  //   1) We may actually want a block here so we can fail on startup.
  //   2) We could poll for completion of this and after a certain amount of 
  //      time either give up and continue with a warning, or fail.
  ...
  // NOTE: When "direct: true", nodemailer.verify() doesn't ever appear to return
  if (smtpHost.direct) EmailNotifier.verified = true;

  logger.debug('Email notifications initialized, but verification of settings not yet completed');
  logger.debug('  host: ' + ((smtpHost.host) ? smtpHost.host : 'direct connection (not recommended)'));
