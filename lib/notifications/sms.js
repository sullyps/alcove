...
function init(from, to, key, secret, region, maxPrice, smsType)
{
  ...
  if(from) smsAttributes.set('AWS.SNS.SMS.SenderID', from);
  if(maxPrice) smsAttributes.set('AWS.SNS.SMS.MaxPrice', maxPrice);
  if(smsType)smsAttributes.set('AWS.SNS.SMS.SMSType', smsType);
  ...
  SmsNotifier = {
    ...
    send: function(message) {
      var params = {
        ...
        MessageAttributes: (smsAttributes.size > 0 ? smsAttributes : undefined)
      };
