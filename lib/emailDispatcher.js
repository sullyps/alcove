...
module.exports = emailDispatcher = {
  ...
  getNextSummaryEmailTime: function (schedule, date) {
    ...
    var found = false;
    // TODO: Sort in case the days aren't in order.
    ...
    if (!found) {
      numberOfDaysFromDate = 7 - day + scheduledDays[0];
    }
  ...
  backupErrorEmail: function (machine, rsyncInfo, exitCode) {
    var mailOptions = {
      ...
      html: '<p>While backing up the machine ' + machine.name + ' during a scheduled backup,' +
          ' rsync exited with code <em>' + exitCode + '</em> with the following error message.\n\n<p style="color:red;">' +
          rsyncInfo.stderr + '</p>.\n' + 'The backup attempt occurred at <b>' + rsyncInfo.startTime + '</b>.\n\n' +
          'Please resolve this issue before the next scheduled attempt.  You may be able to look in the log files to help ' +
          'resolve this problem.  Thank you.</p>'
    }
  ...
  backupSummaryEmail: function() {
    ...
    db.BackupEvent.findAll({
     where: {
      ...
      var emailSummary = 'Backups Summary\n';
      emailSummary += '  Total size of all backups: ' + backupsDirSize + '\n  Disk space remaining: ' +
          freeSpace + '\n\n';
      for (var i=0; i<machines.length; i++) {
        ...
        for (var j=0; j<backupEvents.length; j++) {
          ...
          if (data.machine === machine.name) {
            emailSummary += '  Date: ' + data.backupTime +
            ', rsync exit code: ' + data.rsyncExitCode.toFixed(2) +
            ', Transfer time: ' + (data.transferTimeSec/60).toFixed(2) +
            ' minutes,  Transfer size: ' + (data.transferSize/(1024*1024*1024)).toFixed(2) + ' GB\n';
          }
        }
      }
    ...
    //TODO:  perhaps list the ones that failed separately.
