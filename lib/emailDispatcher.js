// FIXME: This file for reference only. Will be removed once all other code is completed
...
/**
 * Private helper method that returns the size of the directory and it's contents as a string.
 */
...
/**
 * Private helper method that returns the available disk space as a string.
 */
...
/**
 * Private method to ensure at least one email recipient is configured.
 */
function checkForRecipients()
{
  // TODO: verify all contents of this Array are valid email addresses
  // OR maybe do that in the startup / config?
  if (config.notifications.receiving_email_addr.length > 0) return true;

  // Warn the user
  logger.warn('Attempted to send email with no recipients');
  return false;
}
