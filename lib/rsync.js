...
module.exports = rsync = {
	 ...
	 * 	 Whether or not this backup was requested by the user
	 * @param callback
	 *   The callback function(error, statisticsObj).
	 */
	runRsync: (config, machine, backupRequested, callback) => {
		...
		rsyncWrapper(rsyncOptions, (error, stdout, stderr, cmd) => {
			...
			if (error) {
				...
				// Append '-requested' to the directory name in the event that this backup was requested by the end user
				if (backupRequested) newDirectory += '-requested';
