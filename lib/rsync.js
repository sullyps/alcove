...
module.exports = rsync  =  {
  ...
  runRsync: (config, machine, callback) => {
    ...
    // Ensure dir
    // TODO: empty out the directory too (in case it was part of a previous partial backup)
    try
    {
   ...
   * convention.
   * TODO: Discuss whether this should be a 'private' method
   * It makes sense to export so then system.js has access to validate
   * dirnames, then there wouldn't be two copies of the same function.
   * However, this does not necessarily follow good programming practice.
   *
   * @param Backup directory dirname
   */
   ...
   */
   getLastBackupDir: (machineBackupDir) => {
     ...
     dirs.forEach((dirname) => {
       ...
       if (rsync.isValidDirname(dirname) && fs.lstatSync(fullPath).isDirectory())
       {
