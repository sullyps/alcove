...
module.exports = rsync  =  {
  ...
  runRsync: function (config, machine, callback) {
    ...
    for (var i=0; i< machine.topLevelInclusion.length; i++) {
      var pattern = user + '@' + machineIP + ':' + machine.topLevelInclusion[i];
      machineInclusionPatterns.push(pattern);

      logger.debug('Machine Inclusion Pattern: ' + machineInclusionPatterns);
    }

    // TODO: Check for undefined config options and create valid arrays 
    // before defining object
    // The order of options are src, then excludeFirst, include, and finally 
    // exclude patterns. This is stated in the rsyncwrapper documentation - <link>
    logger.trace('Setting up the rsync options.');
    var rsyncOptions = {
      src: user + '@' + machineIP,
      excludeFirst: machine.extensionExclusion.concat(machine.fileExclusion),
      include: machineInclusionPatterns,
      exclude: ['"*"'],
    ...
    if (config.rsync.identity) 
      rsyncOptions.privateKey = 'etc/backup/' + config.rsync.identity;
