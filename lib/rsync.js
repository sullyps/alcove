...
module.exports = rsync  =  {
  ...
  runRsync: function (config, machine, callback) {
    ...
    var excludeFirst;
    if (machine.extensionExclusion && machine.fileInclusion)
    ...
    else if (machine.fileInclusion)
      excludeFirst = machine.fileInclusion;
   ...
   */
   getLastBackupDir: function(machineBackupDir) {
     ...
     dirs.forEach(function(dirname) {
       ...
       if (dirname.match(/^(19|20)\d\d\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])T\d\d\:\d\d\:\d\d\.\d\d\dZ$/) && fs.lstatSync(dirname).isDirectory())
       {
