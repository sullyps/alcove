...
function validateBackupSchedule(schedule)
{
  ...
  if (schedule.match(/^(([0-6,]){1,13}\(\d\)\|*)+\;\[\d{1,2}\:[0-5]\d\]$/))
  {
    ...
    errs.push('Backup schedule :' + schedule + ' does not match d,d,d(N)|d,d,d,d(N);[hh:mm] format');
  return errs;
}
 ...
 * TODO: Separate machine config parsing and validation from backup.
 */
function validateConfig(config, spec)
{
  ...
  // Push all errors from notification parsing onto the error string
  var notificationErrs = validateNotifications(config);
  if (notificationErrs.length)
    errs = errs.concat(notificationErrs);
...
function validateNotifications(config)
{
  ...
  // RFC 5322 Regex for validating emails - https://stackoverflow.com/questio    ns/201323/how-to-validate-an-email-address-using-a-regular-expression
  if (config.notifications.email_to)
  {
    var emails = config.notifications.email_to;
    ...
    if (!Array.isArray(emails))
      errs.push('config.notifications.email_to configuration *must* be in ' +
        'an array');
    else
    {
  ...
  // The recipient must be specified in one of two formats for SMS to
  // configure properly.
  if (config.notifications.sms_to)
  {
    var smsTo = config.notifications.sms_to;

    if (Array.isArray(smsTo))
      errs.push('The SMS recipient configuration should *not* be an array!');

    // NOTE: The following regex may change when/if AWS adds more regions or
    // changes how the ARNs are generated
    else if (smsTo.match(/^arn\:aws\:sns\:(us|ap|ca|eu|sa)\-(east|west|north|south|central|northeast|northwest|southwest|southeast)\-[1-3]\:\d{12}\:[a-zA-z\-\_\d]{1,256}/))
      config.notifications.sms_type = 'topicArn';

    // All valid phone numbers must be in E.164 format -
    // [+] [country code] [subscriber number including area code]
    // and can have a maximum of fifteen digits. AWS SNS will accept a
    // number without the prefixed '+'.
    else if (smsTo.match(/^\+?[1-9]\d{10,14}$/))
      config.notifications.sms_type = 'phoneNum';

    else
    {
      errs.push(smsTo + ' is an invalid sms recipient address');
  ...
  if (config.notifications.sms_region)
  {
    ...
    if (Array.isArray(region))
      errs.push('The AWS sms_region should *not* be an array!');
    else if (!region.match(/^(us|ap|ca|cn|eu|sa)\-(east|west|north|south|central|northeast|northwest|southeast|southwest)\-[1-3]$/))
    {
      errs.push(region + ' is not a valid AWS region');
    }
  }
  return errs;
}
