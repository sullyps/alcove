...
/**
 * Validate given string is a positive integer.
 */
...
function validateConfig(config)
{
  ...
  if (!isPositiveInteger(config.port) || Math.floor(Number(config.port)) > 65535)
  {
    errs.push('Port must be a positive integer from 0 to 65535 in configuration');
  }
  if (!isPositiveInteger(config.rsync.max_simultaneous))
  {
    errs.push('Max simultaneous rsync processes must be a positive integer');
  }
  if (!isPositiveInteger(config.rsync.retry.max_attempts))
  {
    errs.push('Max attempts must be a positive integer value');
  }
...
function validateNotifications(config)
{
  ...
  if (config.sms)
  {
    ...
    if (config.sms.sms_to && (!config.sms.access_key || !config.sms.secret_key || !config.sms.aws_region))
    {
