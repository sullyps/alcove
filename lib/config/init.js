...
function readConfig(filename)
{
  ...
  {
    sshHostTestPromises.push(new Promise((resolve, reject) => {
      child_process.exec(`ssh-keyscan -H ${machineConfig.host}`, { timeout: 5000 }, (error, stdout, stderr) => {
        console.log();
        console.log(`${machineConfig.name}: ${machineConfig.host}`);
