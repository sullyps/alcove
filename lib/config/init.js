...
const SPEC = {
  ...
  machines: {
    valid: [
      "name", "host", "port", "backup_directories", "ignore_extensions",
      "ignore_files", "schedule"
    ],
    required: [
      "name", "host", "port", "backup_directories", "schedule"
    ]
  }
};
...
function validateBackupSchedule(schedule)
{
  ...
  if (schedule.match(/^(([0-6,\-]){1,13}\([0-9]*\)\|*)+\;\[\d{1,2}\:[0-5]\d\]$/))
  {
    try
    {
  ...
  paths.forEach(testPath => {
    if (fs.existsSync(testPath) && fs.existsSync(testPath + ".pub"))
    {
      ...
      try
      {
        ...
        // [NOTE FOR NEXT COMMIT (sully-devel branch)]
        /*
        The 2 lines that were commented out below were causing problems. Even when I gave read/write
        permissions to everyone, this line would always throw the 'wrong permissions' error. I've commented it out for now,
        but I genuinely have no clue what's wrong with this. Furthermore, at this time, the SSH keys are only able to be found
        in the filesystem when running `gulp` directly instead of the `npm start` script, which is assigned to only run
        the `gulp` command and nothing else.
        // TODO: Uncomment this and figure out what's wrong
        */
...
{
  // TODO: This type of check won't work if "StrictHostKeyChecking no" and there
  //   isn't a "known_hosts" file.  I think it would be better to actually 
  //   attempt to connect instead of looking for a match in this file,
  //   depending on the configuration this failure might not actually stop us
  //   from connecting to the machine (granted in a less secure way).
  return true;
  /*
...
function validateMachineConfig(machineName, config)
{
  ...
  if (Number(config.port) !== 22 && (Number(config.port) < 1024 || Number(config.port) > 65535))
  {
    errs.push(`Invalid port (${config.port}) for machine '${machineName}': SSH server port must 22 or between 1024 and 65535`);
  }
