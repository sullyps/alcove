...
/**
 * Validate the config options for a machine specific config. This method
 * additionally transforms a few options so that they will be easier to use
 * with rsync. 
 */
function validateMachineConfig(machineName, config)
{
  ...
  // Now validate and transform the rsync include/exclude settings
  // NOTE: We are requiring the config to have a simplified set of values for
  //   the complicated rsync include/exclude rules. In this 
  // TODO
}
...
function addDefaults(defaults, target)
{
  ...
  for (let key in defaults)
  {
    ...
    {
      // Ensure we merge both Arrays if present
      if (target[key] === undefined) target[key] = defaults[key];
      else target[key] = defaults[key].concat(target[key]);
    }
