...
const SPEC = {
  valid: [ 
    ...
    "notifications.sms_region", "notifications.sms_to", "notifications.which"
  ],
  required: [
    "ip", "port", "log_dir", "log_level", "data_dir",
  ...
  machines: {
    valid: [
      "name", "ip", "topLevelInclusion", "extensionExclusion", "fileExclusion", "schedule"
    ],
    required: [
      "name", "ip", "schedule"
    ]
...
function validateConfig(config, spec)
{
  ...
  if (config.notifications.sms_to)
  {
    ...
    if (smsTo.match(/^arn\:aws\:sns\:(us|ap|ca|eu|sa)\-(east|west|north|south|central|northeast|northwest|southwest|southeast)\-[1-3]\:\d{12}\:[a-zA-z\-\_\d]{1,256}/))
      config.notifications.which = 'topicArn';
    else if (smsTo.match(/^\d{11}$/))
      config.notifications.which = 'phoneNum';
    else
    {
      errs.push('Invalid notifications sms recepient field.');
      config.notifications.which = '';
    }
  }
