...
try {
  ...
  // Logging is not set up yet since it requires the configuration to be set.
  console.error('Please make sure that you have a backup.ini file at ' + rootPath + '/etc/backup/backup.ini.' +
  '\n    To get up and running, copy the contents of backup.example.ini into a file named backup.ini and change the parameters ' +
...
module.exports = {
  configureMachines: function(app) {
    ...
    fs.readdir(machineConfPath, function(error, files) {
      ...
      {
        throw new Error('Backup destination ' + destinationDirectory + ' defined in backup.ini must exist before running the application.');
      }
