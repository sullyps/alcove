      ...
      if (config.notifications) 
      {
            ...
            config.notifications.sms_secretKey && config.notifications.sms_region)
        {
          try
          {
            ...
            if (smsTo.match(/arn\:aws\:sns\:(us|ap|ca|eu|sa)\-(east|west|north|south|central|northeast|northwest|southwest|southeast)\-\d\:\d{12}\:[A-z\-]+/))
            {
              which = 'topicArn';
            }
            else if (smsTo.match(/^d{10}/))
            {
              which = 'phoneNum';
            }
            else
            {
              throw new Error('Invalid sms_to field in config file');
            }
            dispatchers.push(require('./notifications/sms')(smsTo, accessKeyId, secretAccessKey, region, which));
          }
          catch (error)
          {
